...
//				 	 				_this._showUserList();
				 	 			...
				 	 			// 绑定选中事件
				 	 			_this.userList.find('.mod-at-list').find('li').hover(
				 	 					function()
				 	 					{
				 	 						$(this).addClass('hover');	
				 	 					},
...
//					core.at._createUserlistDiv(user);
	...
	_createUserlistDiv: function(user) 
	{
		...
		// 数据模板
		if(!user && document.getElementById('atUserList') == null)
		{   //@后跟了查找不到用户的字符时不显示提示框2012/10/12
	...
	_showUserList: function() 
	{
        ...
        this.userList.css(
        		{
        			'left':(x.left-1)+'px','top':(x.top+this.textarea.height()+10)+'px','width':this.textarea.width()+10+'px','display':'block'
        		});
        return false;
	},
	/**
