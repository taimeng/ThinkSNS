...
class FeedListWidget extends Widget {
    ...
    /**
     * @param string type 获取哪类活动 following:我关注的 space：
     * @param string feed_type 活动类型
     * @param string feed_key 活动关键字
    ...
    /**
     * 获取活动数据，渲染活动显示页面
     * @param array $var 活动数据相关参数
     * @param string $tpl 渲染的模板
     * @return array 获取活动相关模板数据
     */
    private function getData($var, $tpl = 'FeedList.html') {
    	...
    	$map = $list = array();
    	$type = $var['new'] ? 'new'.$var['type'] : $var['type'];	// 最新的活动与默认活动类型一一对应
    	...
    	switch($type) {
    		...
    		case 'all'://所有的 --正在发生的
    			if(!empty($var['feed_key'])){
                    ...
                    //设定可查看的全站活动总数，可以提高大数据量下的查询效率
                    $max = 10000;
    				$list = model('Feed')->getList($where,$this->limitnums,'',$max);
    			}
    			break;
    		case 'newfollowing'://关注的人的最新活动
                $where ='( a.is_audit=1 OR ( a.is_audit=0 AND a.uid='.$GLOBALS['ts']['mid'].') ) AND a.is_del = 0 ';
    			if($var['maxId'] > 0){
