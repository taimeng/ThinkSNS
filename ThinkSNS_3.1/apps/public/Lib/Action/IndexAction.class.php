...
class IndexAction extends Action 
{
	...
	public function index()
	{
		...
		if (false) // For test
		{
			...
			$className0 = 'SendWeiboWidget';
			$className1 = 'SendActivityWidget';
		}	
		...
		$initHtml = $weiboSet['weibo_default_topic'];		// 活动框默认话题
		if($initHtml)
		{
			$initHtml = '#'.$initHtml.'#';
		}
		...
		switch($d['type']) 
		{
			case 'all':
				$this->setTitle('全部活动');
				$this->setKeywords('全部活动');
				break;
			case 'channel':
				$this->setTitle('我的萌圈');
				$this->setKeywords('我的萌圈');
				break;
			default:
	...
	public function following() 
	{
		...
		$groupList = array(array('gid'=>0, 'title'=>'全部'), 
				array('gid'=>-1, 'title'=>'相互关注'), 
				array('gid'=>-2, 'title'=>'未分组'));
